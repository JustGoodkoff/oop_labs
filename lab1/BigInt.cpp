...
/**                             BITWISE OPERATORS                             **/
    ...
    std::string fromDecToBin(std::string str) {
        ...
        if (cur < BigInt(0)) {
            str.erase(str.begin());
            binNotation = BigIntNS::fromDecToBin(str);
            for (size_t i = 0; i < binNotation.size(); i++) {
                if (binNotation[i] == '1') {
                    binNotation[i] = '0';
            ...
            binNotation = BigIntNS::fromDecToBin(binNotation);
    ...
    std::string fromBinToDec(std::string str) {
        ...
        if (str[0] == '-') {
            ...
            str.insert(0, 1, '1');
            BigInt current(str);
            str = std::string(BigInt(BigIntNS::fromBinToDec(str)) - BigInt(1));
            str = BigIntNS::fromDecToBin(str);
            str.erase(str.begin());
            BigIntNS::binInverse(str);
            decadeNotation = -BigInt(BigIntNS::fromBinToDec(str));
        } else {
